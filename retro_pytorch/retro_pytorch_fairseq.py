...
class Decoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        wrapper = partial(PostNorm, dim, scale_residual = 1., norm_klass = LayerNorm)
        ...
        for layer_num in range(1, depth + 1):
            ...
            self.layers.append(nn.ModuleList([
                ...
                # LayerNorm(dim) if has_cross_attn else None,
                wrapper(FeedForward(dim = dim, mult = ff_mult, dropout = ff_dropout)),
                # LayerNorm(dim),
            ]))
...
class RETRO(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.pos_emb = PositionalEmbedding(max_seq_len, enc_dim, pad_id, learned=False)

        self.chunk_size = chunk_size

        assert enc_dim == dec_dim # for fairseq model
